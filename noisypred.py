...
def add_noise(ys: np.ndarray, sn_ratio ):
    new_ys = ys.copy()
    new_ys[:] += np.random.normal(size=ys.shape) * np.std(ys) / sn_ratio
    return new_ys
...
if __name__ == "__main__":
    ...
    true_y = f(X)
    y = add_noise(true_y, sn_ratio=sn_ratio)
    ...
    for train, test in kf.split(X, y):
        ...
        #true_y_std = np.std(true_y[test])
        #score = sklearn.metrics.mean_squared_error(true_y[test] / true_y_std, pred_y / true_y_std)
        score = sklearn.metrics.accuracy_score(np.sign(true_y[test]), np.sign(pred_y))
        #print(pred_y, y[test])
        #print(y[test])
        scores.append(score)
